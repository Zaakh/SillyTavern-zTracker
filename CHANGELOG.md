...
- Text-completion tracker-generation requests now inject SillyTavern's active user-alignment message before assistant-opening chats, so instruct-mode story string prefixes such as `[INST]` apply to the opening tracker prompt instead of sending the initial tracker system block as raw text.
