...
| Text completion, assistant role, terminal tracked user in a confirmed single-speaker solo chat | Raw assistant text block at the end of the prompt with `ignoreInstruct: true` and a synthesized real-assistant reply cue | The tracker label stays in `content`; zTracker does not set `name` in this raw fallback |
...
- `src/ui/ui-init.ts` passes the host hints that distinguish text-completion-safe mode and group-chat mode.