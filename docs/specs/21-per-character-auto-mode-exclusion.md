...
> **Multi-module update (2026-08-10):** this design predates the multi-module tracker feature and describes a single implicit exclusion flag. The shipped implementation instead stores exclusion per tracker Module id (`autoModeExclusions: Record<moduleId, boolean>` on the character card, see `src/ui/character-auto-mode-exclusion.ts`). The character-panel toggle button acts as a single kill-switch: one click sets the exclusion flag uniformly across every currently configured Module, and the button's excluded/included display reflects whether the character is excluded from *all* configured Modules. A character excluded from only some configured Modules displays as "included" (with a distinct tooltip naming the partial state) until the next click normalizes it to fully excluded. See the `fix-character-exclusion-all-modules` OpenSpec change for the bug this corrected (the toggle previously wrote/read only the default Module's exclusion flag).
>
> **Enabled vs. disabled Modules:** the toggle's *write* scope includes disabled Modules (so a disabled Module's exclusion intent survives being re-enabled later), but its *display* and *toggle-direction* read scope is enabled Modules only — the button describes the character's actual current auto-mode exposure, not a Module that cannot generate anyway. Module ids are re-resolved from live settings at both button-sync time and at click time (not cached from when the button was first created), so adding, removing, or re-enabling a Module while the character panel stays open is reflected on the very next click without needing to reopen the panel.
>
> **Group chats:** outgoing auto-mode exclusion only applies to solo chats (there is no single "active chat character" to check in a group chat), so a group member's exclusion flag only suppresses that member's own incoming `CHARACTER_MESSAGE_RENDERED` auto-generation, not any outgoing generation. This matches the original design's group-chat scope and is unchanged by the multi-module update.
